function [speaker_ivector_models, speaker_model_size, verify_ivectors, verify_labels] = separate_enrol_verify(enrol_verify_ivectors, proportion)
    ...
    parfor speakerIdx = 1:size(unique_speakers,1)
        ...
        speaker_ivector_models = [speaker_ivector_models mean(model_ivectors,2)];
        speaker_model_size = [speaker_model_size num_for_model];
        other_ivectors = cat(2,speaker_ivectors{num_for_model+1:end, 1});
        verify_ivectors = [verify_ivectors other_ivectors];
        verify_labels = [verify_labels repmat(unique_speakers(speakerIdx), 1, size(other_ivectors,2))];
    end
end
